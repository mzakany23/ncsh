...
def run_backfill(start_year=2007, start_month=1, end_year=None, end_month=None,
    ...
    # Table name parameter is now ignored as we only support file-based lookups
    if table_name is not None:
        logger.warning("The table_name parameter is deprecated and will be ignored. Using file-based lookup instead.")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        bucket_name = os.environ.get('DATA_BUCKET', 'ncsh-app-data')
        # Table name is no longer needed as we're using file-based lookup
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lookup-type', choices=['file'], default='file', help='Lookup type (only file is supported)')
