#!/usr/bin/env python3
"""
Utility script to convert JSON format from backfill spider format to the format expected by processing Lambda.
This script:
...
def convert_json_format(source_file: str, dest_file: str) -> int:
    """
    Convert JSON format from backfill spider format to processing Lambda format.
    ...
    try:
        ...
        if isinstance(source_data, dict) and 'games' in source_data:
            ...
            # This is the format from the regular scrapy spider
...
def main():
    ...
    parser = argparse.ArgumentParser(description='Convert JSON format from backfill spider to processing Lambda')
    ...
    if args.upload:
        if not args.bucket:
            logger.error("--bucket is required for --upload")
            return 1
        
        logger.info(f"Uploading converted files to s3://{args.bucket}/{args.prefix}")
        uploaded_keys = upload_to_s3(args.dest_dir, args.bucket, args.prefix)
        logger.info(f"Upload complete: {len(uploaded_keys)} files uploaded")
    
    # Trigger processing if requested
    if args.process:
        if not args.bucket:
            logger.error("--bucket is required for --process")
            return 1
        
        if not args.dst_bucket:
            logger.info("--dst-bucket not specified, using source bucket")
            args.dst_bucket = args.bucket
        
        if not uploaded_keys:
            logger.error("No files were uploaded to process")
            return 1
        
        logger.info(f"Triggering processing Lambda for {len(uploaded_keys)} files")
        response = trigger_processing(args.bucket, uploaded_keys, args.dst_bucket, args.dst_prefix)
