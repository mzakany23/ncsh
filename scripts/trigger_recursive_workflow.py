...
def main():
    parser = argparse.ArgumentParser(description='Trigger NC Soccer Scraper Recursive Workflow')
    ...
    parser.add_argument('--batch-size', type=int, default=10,
                      help='Number of days per batch (default: 10)')
    ...
    # If state machine ARN was not provided, use the default recursive workflow
    state_machine_arn = args.state_machine_arn
    if not state_machine_arn:
        state_machine_arn = f"arn:aws:states:{args.region}:552336166511:stateMachine:ncsoccer-unified-workflow-recursive"
